...
#include "pano_util.h"
  ...
  Rect computeSearchBox(DirectionHint hint) {
    ...
    switch (hint) {
      ...
      case RIGHT: return Rect(0, -ortho, dist + 1, thickness);
      case DOWN: return Rect(-ortho, 0, thickness, dist + 1);
      case LEFT: return Rect(-dist, -ortho, dist + 1, thickness);
      case UP: return Rect(-ortho, -dist, thickness, dist + 1);
      case UNKNOWN: break; // silence warning
    }
    return Rect();
  }
...
void generateNovelView(
  ...
  outNovelViewFromL = generateNovelViewSimpleCvRemap(
    imageL, flowRtoL, shiftFromL);
  outNovelViewFromR = generateNovelViewSimpleCvRemap(
    imageR, flowLtoR, 1.0 - shiftFromL);
  // imwrite("/home/yang/pano_pic/test1.png",outNovelViewFromL);
  // imwrite("/home/yang/pano_pic/test3.png",flattenMat);
  Rect r(0,outNovelViewFromL.rows*2/3,outNovelViewFromL.cols,outNovelViewFromL.rows/6);
  get_brightness_foralpha(outNovelViewFromR(r),outNovelViewFromL(r));
  outNovelViewFromL= ajust_brightness_foralpha(outNovelViewFromL);
  // double new_shift=0.6;
  // outNovelViewMerged = combineNovelViews(
  //   outNovelViewFromL, 1.0 - shiftFromL,
  //   outNovelViewFromR, shiftFromL,
  //   flowLtoR, flowRtoL);
  outNovelViewMerged=Mat(outNovelViewFromR.size(),outNovelViewFromR.type());
  double alpha=0.0f;
  for (int i = 0; i < outNovelViewMerged.cols; ++i)
  {
    for (int j = 0; j < outNovelViewMerged.rows; ++j)
    { 
      if(j<outNovelViewMerged.rows*5/8)
      {
        outNovelViewMerged.at<Vec4b>(j,i)=outNovelViewFromL.at<Vec4b>(j,i);
      }
      else if(j<outNovelViewMerged.rows*5/8+200)
      { 
        alpha=((float)j-(float)outNovelViewMerged.rows*5/8.0f)/200.0f;
        outNovelViewMerged.at<Vec4b>(j,i)[0]=outNovelViewFromL.at<Vec4b>(j,i)[0]*(1-alpha)+outNovelViewFromR.at<Vec4b>(j,i)[0]*alpha;
        outNovelViewMerged.at<Vec4b>(j,i)[1]=outNovelViewFromL.at<Vec4b>(j,i)[1]*(1-alpha)+outNovelViewFromR.at<Vec4b>(j,i)[1]*alpha;
        outNovelViewMerged.at<Vec4b>(j,i)[2]=outNovelViewFromL.at<Vec4b>(j,i)[2]*(1-alpha)+outNovelViewFromR.at<Vec4b>(j,i)[2]*alpha;
        outNovelViewMerged.at<Vec4b>(j,i)[3]=255;
      }
      else if(j<outNovelViewMerged.rows)
     {
        outNovelViewMerged.at<Vec4b>(j,i)=outNovelViewFromR.at<Vec4b>(j,i);
      }
    }
  }
  // imwrite("/home/yang/pano_pic/test3.png",mergedImage);
  //   outNovelViewMerged = combineNovelViews(
  //   outNovelViewFromL, 1.0 - new_shift,
  //   outNovelViewFromR, new_shift,
  //   flowLtoR, flowRtoL);
...
void patchMatchPropagationAndSearch(
  ...
  Mat I0x, I0y, I1x, I1y;
    ...
    if (flow.empty()) {
      ...
      if (MaxPercentage > 0 && hint != UNKNOWN) {
 ...
 float errorFunction(
  ...
  { 
    ...
    Mat flowUtoD,flowDtoU;
    DirectionHint d1=UP;
    DirectionHint d2=DOWN;
    // Mat mat1,mat2,mat3;
    computeOpticalFlow(img1,img2,Mat(),Mat(),Mat(),flowDtoU,d1);
    computeOpticalFlow(img2,img1,Mat(),Mat(),Mat(),flowUtoD,d2);
    double shiftFromL=0.80;
    Mat  outNovelViewMerged,outNovelViewFromL,outNovelViewFromR;
    generateNovelView(
      shiftFromL,
      outNovelViewMerged,
      outNovelViewFromL,
      outNovelViewFromR,img1,img2,flowUtoD,flowDtoU);
    return outNovelViewMerged;
    // Mat greypic=visualizeFlowAsGreyDisparity(flowDtoU);
    // imwrite(argv[3],outNovelViewMerged);
    // return 0;
  }